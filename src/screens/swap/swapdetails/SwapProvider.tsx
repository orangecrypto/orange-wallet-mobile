  ...
  return (
    <View style={[styles.card, styles.selectedCard]}>
      ...
      <View style={styles.itemContainerRow}>
        <View style={styles.itemRow}>
          {item?.receiveIcon ? (
            <Image source={item.receiveIcon} style={styles.btcIcon} />
          ) : (
            <TokenImage fungibleToken={item} size={40} round variant="dark" />
          )}
          <Text style={styles.providerName}>
            {item.value} <Text style={styles.tick}>{item.ticker}</Text>
          </Text>
        </View>
        <View style={styles.itemRow}>
          ...
          <Text style={styles.fiat}>
            {` ~${item.fiatRate}`}
            <Text style={[styles.fiat, { fontSize: Responsive.size12 }]}>{` USD`}</Text>
          </Text>
        </View>
      </View>
    </View>
